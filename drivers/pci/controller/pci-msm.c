...
static void msm_pcie_loopback(struct msm_pcie_dev_t *dev, bool local)
{
	...
	if (dma_set_mask_and_coherent(&dev->pdev->dev, ~0ULL)) {
...
static void msm_pcie_lock_init(struct msm_pcie_dev_t *pcie_dev)
{
	...
	scnprintf(rc_name, MAX_RC_NAME_LEN, "pcie%d-short", pcie_dev->rc_idx);
	pcie_dev->ipc_log =
		ipc_log_context_create(PCIE_LOG_PAGES, rc_name, 0);
	if (pcie_dev->ipc_log == NULL)
		pr_err("%s: unable to create IPC log context for %s\n",
		...
		PCIE_DBG(pcie_dev,
			"PCIe IPC logging is enable for RC%d\n",
			pcie_dev->rc_idx);
	scnprintf(rc_name, MAX_RC_NAME_LEN, "pcie%d-long", pcie_dev->rc_idx);
	pcie_dev->ipc_log_long =
		ipc_log_context_create(PCIE_LOG_PAGES, rc_name, 0);
	if (pcie_dev->ipc_log_long == NULL)
		pr_err("%s: unable to create IPC log context for %s\n",
		...
		PCIE_DBG(pcie_dev,
			"PCIe IPC logging %s is enable for RC%d\n",
			rc_name, pcie_dev->rc_idx);
	scnprintf(rc_name, MAX_RC_NAME_LEN, "pcie%d-dump", pcie_dev->rc_idx);
	pcie_dev->ipc_log_dump =
		ipc_log_context_create(PCIE_LOG_PAGES, rc_name, 0);
	if (pcie_dev->ipc_log_dump == NULL)
		pr_err("%s: unable to create IPC log context for %s\n",
...
#ifdef CONFIG_SEC_PCIE_L1SS
	mutex_init(&pcie_dev->l1ss_ctrl_lock);
	pcie_dev->l1ss_disable_flag = 0;
